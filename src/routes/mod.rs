...
/// Create stateless routes for the Pos module.
///
...
///
/// ```ignore
/// let module = PosModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::PosModule) -> Router<()> {
    Router::new()
        .merge(create_pos_closing_entry_routes(module.pos_closing_entry_service.clone()))
        .merge(create_pos_invoice_routes(module.pos_invoice_service.clone()))
        .merge(create_pos_invoice_item_routes(module.pos_invoice_item_service.clone()))
...
pub fn get_routes(module: &crate::PosModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Pos module.
///
...
pub fn create_combined_routes(module: &crate::PosModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Pos module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::PosModule) -> Router<AppState> {
    Router::new()
