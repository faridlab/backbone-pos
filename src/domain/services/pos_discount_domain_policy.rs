//! Domain policy for PosDiscount aggregate
//!
...
/// Domain policy for PosDiscount — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PosDiscount>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PosDiscountDomainPolicy = PermitAllPolicy<PosDiscount>;
