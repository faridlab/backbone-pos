//! Domain policy for PosManagerPin aggregate
//!
...
/// Domain policy for PosManagerPin — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PosManagerPin>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PosManagerPinDomainPolicy = PermitAllPolicy<PosManagerPin>;
