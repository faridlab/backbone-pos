//! Domain policy for PosTable aggregate
//!
...
/// Domain policy for PosTable — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PosTable>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PosTableDomainPolicy = PermitAllPolicy<PosTable>;
