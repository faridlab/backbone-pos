//! Domain policy for PosClosingEntry aggregate
//!
...
/// Domain policy for PosClosingEntry — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PosClosingEntry>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PosClosingEntryDomainPolicy = PermitAllPolicy<PosClosingEntry>;
