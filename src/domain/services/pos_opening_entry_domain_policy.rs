//! Domain policy for PosOpeningEntry aggregate
//!
...
/// Domain policy for PosOpeningEntry — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PosOpeningEntry>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PosOpeningEntryDomainPolicy = PermitAllPolicy<PosOpeningEntry>;
