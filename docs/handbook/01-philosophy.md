...
## This module's north star — orchestrate, don't re-implement

`backbone-pos` is the **retail counter path of an Indonesia-first ERP's Financials pillar**. It
owns the cashier session, the sale ticket, tender, and the drawer — but it **posts no general
ledger (GL) itself**. On recognition it *orchestrates* two downstream emitters: it drives
`backbone-billing` for revenue and `backbone-payment` for settlement through outbound ports
(`BillingPort` / `PaymentPort`), so retail reuses the *same* GL posts as web and B2B sales.

Concretely, a cash sale books `Dr A/R · Cr Revenue` (billing raises the real Sales Invoice) and
then `Dr Cash · Cr A/R` (payment settles it) — so **A/R nets to zero at the counter**. POS never
touches the ledger; it composes the emitters that do.

The wire contract is the point: **the shipped POS library has zero normal Cargo edge to billing,
payment, or accounting** (they are dev-dependencies only, for the seam test). The ports *are* the
contract — an envelope + anti-corruption seam that a composition layer implements over the real
services. This "orchestrate, don't re-implement" stance is the north star; everything below is the
framework machinery that keeps it cheap.
...
And the POS-specific non-goals that fall straight out of the north star:

- **POS posts no GL.** There is no parallel posting path in this module. Revenue and settlement are
  emitted by billing and payment; POS only orchestrates them through the ports.
- **PPN / tax is delegated, not computed.** POS *carries* the supplied tax (`tax_total`) into the
  handoff; it does not calculate PPN. The base is tax-free until a profile tax account exists —
  recognition refuses tax otherwise. Tax computation belongs to billing/tax.
- **Several capabilities are deliberately deferred**, not stubbed as scaffolding: consolidated
  billing handoff, offline sync, fiscal-printer output, multi-currency, and partial / line-level
  returns (returns are full-ticket only today). These arrive when a real requirement demands them.
...
entity you add. POS is squarely that shape: its register / session / ticket / line / tender /
Z-report entities live on PostgreSQL, so the module is the right tool — the orchestration seam
sits *on top of* the generated CRUD, not instead of it.
