...
/// Undecorated, the stale-session claim reads the whole table — the per-org fence that used to
/// keep these scheduler runs disjoint is composition-installed (ADR-0029) — so the runs serialize
/// and every assertion discriminates by the session ids the test itself minted.
static ALERT_RUN_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());

fn days_ago(n: i64) -> chrono::NaiveDateTime {
    chrono::Utc::now().naive_utc() - chrono::Duration::days(n)
}

async fn open_at(w: &PosWriteService, prof: Uuid, opened_at: chrono::NaiveDateTime) -> Uuid {
...
async fn stale_session_alerts_once_then_latches() {
    let _serial = ALERT_RUN_LOCK.lock().await;
    let pool = pool().await;
    let rec = support::Recorder::default();
    let w = PosWriteService::with_sink(pool.clone(), std::sync::Arc::new(rec.clone()));
    let (prof, _tax) = support::zero_tax_profile(&pool).await;
    let session = open_at(&w, prof, days_ago(8)).await;

    let alerts = w.alert_old_sessions(default_threshold()).await.unwrap();
    let mine: Vec<_> = alerts.iter().filter(|a| a.opening_entry_id == session).collect();
    assert_eq!(mine.len(), 1);
    // Undecorated run: the legacy company twin echoes nil (no ambient org scope).
    assert_eq!(mine[0].company_id, Uuid::nil());
    assert_eq!(mine[0].pos_profile_id, prof);

    let fired = rec.events.lock().unwrap().iter()
        .filter(|e| matches!(e, PosEvent::PosStaleSessionAlerted(a) if a.opening_entry_id == session))
        .count();
    assert_eq!(fired, 1, "one alert, one event");

    // The latch: a rerun re-alerts nothing.
    let again = w.alert_old_sessions(default_threshold()).await.unwrap();
    assert!(!again.iter().any(|a| a.opening_entry_id == session), "the once-only latch holds");
...
async fn stale_event_carries_the_cashier_party_ref() {
    ...
    let session = w.open_session(NewSession {
        pos_profile_id: prof, branch_id: None, cashier_party_id: cashier,
        opened_at, opening_balances: vec![],
    }).await.unwrap();

    w.alert_old_sessions(default_threshold()).await.unwrap();
    let event = rec.events.lock().unwrap().iter().find_map(|e| match e {
        PosEvent::PosStaleSessionAlerted(a) if a.opening_entry_id == session => Some(a.clone()),
        _ => None,
    }).expect("the alert event fired");
    let expected = PosStaleSessionAlerted {
        opening_entry_id: session, pos_profile_id: prof, company_id: Uuid::nil(),
...
async fn fresh_session_is_not_stale() {
    ...
    let (prof, _tax) = support::zero_tax_profile(&pool).await;
    let session = open_at(&w, prof, days_ago(2)).await;

    let alerts = w.alert_old_sessions(default_threshold()).await.unwrap();
    assert!(!alerts.iter().any(|a| a.opening_entry_id == session), "a fresh session is never claimed");
}
...
async fn closed_session_is_not_alerted() {
    ...
    let w = PosWriteService::new(pool.clone());
    let (prof, _tax) = support::zero_tax_profile(&pool).await;
    let session = open_at(&w, prof, days_ago(9)).await;
    ...
    let alerts = w.alert_old_sessions(default_threshold()).await.unwrap();
    assert!(!alerts.iter().any(|a| a.opening_entry_id == session), "a closed session is never claimed");
}
...
async fn threshold_is_parameterized() {
    ...
    let w = PosWriteService::new(pool.clone());
    let (prof, _tax) = support::zero_tax_profile(&pool).await;
    let session = open_at(&w, prof, days_ago(6)).await;

    let mine = |alerts: &[PosStaleSessionAlerted]| {
        alerts.iter().filter(|a| a.opening_entry_id == session).count()
    };
    assert_eq!(mine(&w.alert_old_sessions(chrono::Duration::days(7)).await.unwrap()), 0);
    assert_eq!(mine(&w.alert_old_sessions(chrono::Duration::days(5)).await.unwrap()), 1);
}

/// SA-6 (tenancy seam): undecorated, the claim has NO filter of its own — one run takes every
/// stale open session it can see, across registers. Keeping scheduler runs disjoint per org unit
/// is composition-installed (ADR-0029): the host enumerates its units and calls the handler once
/// per unit with that unit's scope bound.
#[tokio::test]
async fn undecorated_claim_has_no_filter_of_its_own() {
    let _serial = ALERT_RUN_LOCK.lock().await;
    let pool = pool().await;
    let w = PosWriteService::new(pool.clone());
    let (prof_a, _) = support::zero_tax_profile(&pool).await;
    let (prof_b, _) = support::zero_tax_profile(&pool).await;
    let a_session = open_at(&w, prof_a, days_ago(9)).await;
    let b_session = open_at(&w, prof_b, days_ago(9)).await;

    let alerts = w.alert_old_sessions(default_threshold()).await.unwrap();
    let mut mine: Vec<Uuid> = alerts.iter()
        .map(|a| a.opening_entry_id)
        .filter(|id| *id == a_session || *id == b_session)
        .collect();
    mine.sort_unstable();
    let mut expected = vec![a_session, b_session];
    expected.sort_unstable();
    assert_eq!(mine, expected, "one undecorated run claims every stale session it sees");
}
